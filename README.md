...
GitHub Actions 每天北京时间 17:00 自动执行同一条链路：

1. 抓取中金所 IH / IF / IC / IM 成交持仓排名，生成中信期货和前20机构净多/净空跟踪事件；如果当天数据尚未发布，会自动回退到最近一个已发布交易日，并在备注里写明数据交易日。
...
说明：中金所成交持仓排名属于收盘后数据。北京时间 5:00 只能拿到最近已发布交易日，不能代表当天收盘后的“本日数据”；因此自动任务放在 17:00 执行。
